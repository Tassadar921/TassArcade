...
import type { SerializedEquipmentTypeExtended, SerializedEquipment } from 'backend/types';
...
import DatatableChangeEquipment from '#lib/partials/admin/equipment-type/DatatableChangeEquipment.svelte';
...
export const getEquipmentsColumns = (onSort: (field: string, order: 'asc' | 'desc') => void, onChange: (equipment: SerializedEquipment) => void): ColumnDef<SerializedEquipment>[] => [
    {
        id: 'select',
        header: ({ table }) =>
            renderComponent(Checkbox, {
    ...
    },
    {
        ...
        enableHiding: false,
        cell: ({ row }) =>
            renderComponent(DatatableChangeEquipment, {
                equipment: row.original,
                changeEquipment: onChange,
            }),
    },
];
